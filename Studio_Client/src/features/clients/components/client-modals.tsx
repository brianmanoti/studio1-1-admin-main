...
export function ClientModals({ client, modalType, onClose, onConfirmDelete, isDeleting }) {
  const queryClient = useQueryClient()

  const [formData, setFormData] = useState(client)

  // 🔥 FIX: Ensure modal form updates when selecting another client
  useEffect(() => {
    setFormData(client)
  }, [client])
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => axiosInstance.put(`/api/clients/${client._id}`, data),
    onSuccess: () => {
