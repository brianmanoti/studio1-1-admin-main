...
const {
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => {
      setServerError(err?.response?.data?.message || "Failed to create expense order")
      setIsSubmitting(false)
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: FormData) =>
      axiosInstance
        .put(`/api/expenses/${ExpenseId}`, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        })
        .then((res) => res.data),
    onSuccess: (data) => {
    ...
    onError: (err) => {
      setServerError(err?.response?.data?.message || "Failed to update expense order")
      setIsSubmitting(false)
      setSubmitAttempted(true)
    },
  })
  ...
  useEffect(() => {
    if (!isProjectsLoading && CurrentProjectId && !form.projectId && !ExpenseId) {
      setField("projectId", CurrentProjectId)
    }
  }, [isProjectsLoading, CurrentProjectId, form.projectId, ExpenseId])

  // Auto-select first estimate when estimates load and no estimate is selected
  useEffect(() => {
    if (estimates.length > 0 && !form.estimateId && !ExpenseId) {
  ...
  // Load expense order data when editing - FIXED for nested projectId
  useEffect(() => {
    if (!expenseOrder || !ExpenseId) return
    ...
    const expFormData = {
      ...
      status: expenseOrder.status || "pending",
      date: formatDateToInput(expenseOrder.date),
      deliveryDate: formatDateToInput(expenseOrder.deliveryDate),
      deliveryAddress: expenseOrder.deliveryAddress || "",
      notes: expenseOrder.notes || "",
      vendorName: expenseOrder.vendorName || "",
      vendorContact: expenseOrder.vendorContact || "",
      vendorEmail: expenseOrder.vendorEmail || "",
      vendorPhone: expenseOrder.vendorPhone || "",
      vendorAddress: expenseOrder.vendorAddress || "",
      items: Array.isArray(expenseOrder.items) && expenseOrder.items.length ? 
      ...
      estimateId: expenseOrder.estimateId || "",
      estimateLevel: expenseOrder.estimateLevel || "estimate",
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  const handleBack = () => {
    ...
    else navigate({ to: "/projects/$projectId/expenses" })
  }
  ...
  // Show loading state when fetching expense order data
  if (ExpenseId && isExpenseOrderLoading) {
    return (
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <p className="mt-4 text-gray-600">Loading expense order...</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold text-gray-900">
                {ExpenseId ? "Edit Expense Order" : "Create New Expense Order"}
              </h1>
            </div>
