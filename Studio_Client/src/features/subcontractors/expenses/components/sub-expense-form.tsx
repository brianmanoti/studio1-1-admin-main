...
export default function SubExpenseForm({ expenseId }) {
  ...
  const { data: expenseData, isLoading: isExpenseLoading } = useQuery({
    ...
    onSuccess(expense) {
      ...
      const normalized = {
        ...
        items: Array.isArray(expense?.items) && expense.items.length ? expense.items.map(item => ({
      ...
      if (expense.estimateId) {
        setEstimateData({
          ...
          estimateLevel: expense.estimateLevel || "estimate",
          estimateTargetId: expense.estimateTargetId || ""
        })
      }

      // Set search terms if exists
      if (expense.vendorName) {
        setVendorSearch(expense.vendorName)
      }
      if (expense.subcontractorName) {
        setSubcontractorSearch(expense.subcontractorName)
      }
    },
    onError() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      navigate({ to: `/projects/$projectId/subcontractors/expenses` })
    },
  ...
  const handleDownloadAttachment = async (attachment) => {
    try {
      const response = await axiosInstance.get(`/api/expenses/attachments/${attachment._id}`, {
        responseType: 'blob'
      })
  ...
  const handleRemoveExistingAttachment = async (attachmentId) => {
    ...
    try {
      await axiosInstance.delete(`/api/expenses/attachments/${attachmentId}`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (expenseId) await updateMutation.mutateAsync(formData)
  ...
  // Show loading state while fetching expenseData
  if (expenseId && isExpenseLoading) {
    return (
      <div className="max-w-5xl mx-auto p-8">
        <div className="text-center">Loading Expense record...</div>
      </div>
    )
  }
      ...
      <form onSubmit={handleSubmit} className="w-full mx-auto p-4 md:p-8 space-y-6 bg-white rounded-lg shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-800">
            {expenseId ? `Edit Expense Record - ${expenseData?.expenseNumber || ''}` : "New Expense Record"}
          </h2>
        </div>
        ...
        {/* ExpenseNumber Display */}
        {expenseData?.expenseNumber && (
          <div className="bg-green-50 p-3 rounded border border-green-200">
            <p className="text-green-700 font-medium">ExpenseNumber: {expenseData.expenseNumber}</p>
          </div>
        )}
