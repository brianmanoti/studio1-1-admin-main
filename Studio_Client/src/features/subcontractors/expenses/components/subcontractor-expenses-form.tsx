...
export default function SubcontractorExpensesForm({ expenseId }) {
  ...
  useQuery({
    queryKey: ["subcontractorExpenses", expenseId],
    enabled: !!expenseId,
    queryFn: async () => {
      const res = await axiosInstance.get(`/api/subcontractors/expenses/${expenseId}`)
      return res.data
    },
    onSuccess(expense) {
      const normalized = {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["subcontractorExpenses"] })
  ...
  const updateMutation = useMutation({
    mutationFn: (payload) =>
      axiosInstance.put(`/api/subcontractors/expenses/${expenseId}`, payload).then((res) => res.data),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["subcontractorExpenses", expenseId] })
      ...
      <form onSubmit={handleSubmit} className="max-w-5xl mx-auto p-4 md:p-8 space-y-6 bg-white rounded-lg shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-800">
            {expenseId ? "Edit Expense Record" : "New Expense Record"}
          </h2>
        </div>
