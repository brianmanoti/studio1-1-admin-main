...
export default function EstimateSelector({ onChange }) {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        if (err.response?.status === 404) return [] // no estimates yet
        throw err
      }
    },
    enabled: !!projectId,
  ...
  const allSections = useMemo(
    () => data?.groups?.flatMap((g) => (g.sections || []).map((s) => ({ ...s, groupId: g.key }))) ?? [],
    [data],
  )
  ...
  // ✅ User feedback UI
  if (loadingEstimates || isLoading) return <div className="text-gray-500 text-sm">Loading estimate…</div>
