...
export default function SubcontractorWagesForm({ wageId }) {
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => setServerError(err?.response?.data?.message || "Failed to create wage record"),
  })
  ...
  const updateMutation = useMutation({
    ...
    onError: (err) => setServerError(err?.response?.data?.message || "Failed to update wage record"),
  })
      ...
      <form onSubmit={handleSubmit} className="w-full mx-auto p-4 md:p-8 space-y-6 bg-white rounded-lg shadow-sm">
        ...
        <div className="flex flex-wrap gap-2 justify-end pt-4 border-t">
          {!isLocked && (
            ...
            >
              {isSubmitting ? "Saving…" : wageId ? "Save Changes" : "Create Wage Record"}
            </button>
          )}
          {wageId && (
            <button
