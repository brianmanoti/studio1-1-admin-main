...
export function VendorsTable() {
  ...
  const columns = useMemo(() => [
    ...
    { accessorKey: "createdAt", header: "Created", cell: ({ row }) => new Date(row.getValue("createdAt")).toLocaleDateString("en-KE") },
    {
      id: "actions",
      header: "Actions",
      cell: ({ row }) => {
    ...
    <div className="space-y-4">
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows.length ? table.getRowModel().rows.map(row => (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-24 text-center text-gray-500">No vendors found.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="text-sm text-gray-600">
          ...
          {Math.min((table.getState().pagination.pageIndex + 1) * table.getState().pagination.pageSize, table.getFilteredRowModel().rows.length)} of {table.getFilteredRowModel().rows.length} vendors
        </div>
      ...
      {/* Vendor Modal */}
      <VendorModal vendor={editingVendor} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSaveVendor} />
    </div>
  )
}
